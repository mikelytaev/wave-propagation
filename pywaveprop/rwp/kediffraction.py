from pywaveprop.propagators.ts import ThinScatteringComputationalParams, SpectralIntegrationMethod, Plate, \
    ThinScattering
from pywaveprop.rwp.antennas import *
from pywaveprop.rwp.environment import *
from pywaveprop.rwp.field import *
import math as fm
...
class KnifeEdgeDiffractionCalculator:
    ...
    def __init__(self, src: Source, env: Troposphere, max_range_m, min_range_m=0, dx_m=1, max_propagation_angle=90,
                 x_grid_m=None, z_grid_m=None, p_grid_size=None, inverse=False):
        ...
        width = 4e-5
        eps_r = 1e7
        alpha = 1e-5 * (-1 if inverse else 1)
        bodies = []
