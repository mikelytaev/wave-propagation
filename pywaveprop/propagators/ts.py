...
from pywaveprop.transforms.frft import *
from pywaveprop.transforms.fcc_fourier.fcc import *
from pywaveprop.transforms.filon import *
...
class ThinScattering:
    ...
    def __init__(self, wavelength, bodies, params: ThinScatteringComputationalParams,
                 fur_q_func: Union[types.FunctionType, List[types.FunctionType]], save_debug=False):
        ...
        if fur_q_func is None:
            ...
            if not isinstance(fur_q_func, list):
                fur_q_func = [fur_q_func]
            self.qrc_q = [f(self.p_computational_grid) for f in fur_q_func]
    ...
    def _rhs(self, body_number, i, src_i=0):
        pv, pshv = np.meshgrid(self.p_computational_grid, self.p_computational_grid, indexing='ij')
        _, qshv = np.meshgrid(self.p_computational_grid, self.qrc_q[src_i], indexing='ij')
    ...
    def _super_rhs(self):
        rs = np.empty((self.super_ker_size, self.rhs_num()), dtype=complex)
        ks = len(self.p_computational_grid)
        t = len(self.p_computational_grid) * self.params.quadrature_points
        for src_i in range(0, self.rhs_num()):
            for body_i in range(0, len(self.bodies)):
                for x_i in range(0, len(self.quad_x_grid[body_i])):
                    rs[(body_i*t + ks*x_i):(body_i*t + ks*(x_i + 1)):, src_i] = self._rhs(body_i, x_i, src_i)
        return rs
    ...
    def calculate(self):
        if len(self.bodies) > 0:
            ...
            super_phi = np.empty((len(rhs), self.rhs_num()), dtype=complex)
            for src_i in range(self.rhs_num()):
                super_phi[:, src_i] = sla.solve(left, rhs[:, src_i], overwrite_a=True, overwrite_b=True)
        ...
        for src_i in range(self.rhs_num()):
            ...
            ks = len(self.p_computational_grid)
            t = len(self.p_computational_grid) * self.params.quadrature_points
            psi = self.green_function(self.x_computational_grid, 0, self.p_computational_grid) * self.qrc_q[src_i]
            for body_i in range(0, len(self.bodies)):
                for x_i in range(0, len(self.quad_x_grid[body_i])):
                    phi = super_phi[(body_i*t + ks*x_i):(body_i*t + ks*(x_i + 1)):, src_i]
