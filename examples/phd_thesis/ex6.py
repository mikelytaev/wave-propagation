...
environment.vegetation = [Impediment(x1=0, x2=1000, height=10, material=environment.ground_material)]
...
print('reflection coef: ' + str(abs(real_refl_coef)))
print('reflection coef error: ' + str(abs(abs(computed_refl_coef) - abs(real_refl_coef))))
...
plt = pade_vis.plot2d(min=-25, max=0)
plt.xlabel('Расстояние, км')
plt.ylabel('Высота, м')
plt.tight_layout()
plt.grid(True)
