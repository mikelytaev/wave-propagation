...
#env.knife_edges = [KnifeEdge(range=3e3, height=70)]
...
plt = pade_vis_2.plot_hor_over_terrain(1, pade_vis_joined)
plt.xlabel('Range (km)')
...
pade_vis_2.plot_ver(5.2 * 1E3, ax1, pade_vis_joined)
...
utd_field.field = (np.array(prop_fact).T - 10*fm.log10(0.1) - np.tile(10 * np.log10(utd_field.x_grid), (utd_field.z_grid.shape[0], 1)).T*0) / 2 - 10
utd_vis = FieldVisualiser(utd_field, env=env, trans_func=lambda x: x, label='GO_UTD', x_mult=1E-3)
...
plt = utd_vis.plot_hor_over_terrain(1, pade_vis_joined)
plt.xlabel('Range (km)')
plt.ylabel('10log|u| (dB)')
plt.xlim([0.5, max_range_m*1e-3])
plt.ylim([-100, 0])
...
f, (ax1) = plt.subplots(1, 1, sharey=True)
utd_vis.plot_ver(5.2 * 1E3, ax1, pade_vis_joined)
ax1.set_ylabel('Height (m)')
ax1.set_xlabel('10log|u| (dB)')
ax1.grid()
f.tight_layout()
