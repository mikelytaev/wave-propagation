...
src = GaussSource(freq_hz=50, depth=500, beam_width=90, eval_angle=0)
env = UnderwaterEnvironment()
env.sound_speed_profile_m_s = lambda x, z: z*0 + 1500 #munk_profile(z)
env.bottom_profile = Bathymetry(ranges_m=[0], depths_m=[1000])
env.bottom_sound_speed_m_s = 1700
env.bottom_density_g_cm = 1.5
env.bottom_attenuation_dm_lambda = 0.5

max_ramge = 150000
sspe_comp_params = HelmholtzPropagatorComputationalParams(z_order=4)
sspe_propagator = UnderwaterAcousticsSSPadePropagator(src=src, env=env, max_range_m=max_ramge, comp_params=sspe_comp_params)
