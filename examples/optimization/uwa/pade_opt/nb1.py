...
def tau_error_sup_h(xi, xi_bounds, h, n, dx_wl, pade_coefs):
    xi_grid = np.linspace(xi-h, xi+h, n)
    xi_grid = xi_grid[xi_grid >= xi_bounds[0]]
    xi_grid = xi_grid[xi_grid <= xi_bounds[1]]
    errors = [tau_error(xi, xi2, dx_wl, pade_coefs) for xi2 in xi_grid]
    return max(errors)
...
def precision_step(xi_bounds, k_z_max, dxs_wl: np.array, dzs_wl: np.array, pade_order):
    ...
    for dx_i, dx_wl in enumerate(dxs_wl):
        ...
        for dz_i, dz_wl in enumerate(dzs_wl):
            ...
            error = max([tau_error_sup_h(xi, xi_bounds, h, 100, dx_wl, coefs) for xi in xi_grid])
            res[dx_i, dz_i] = error
    return res
