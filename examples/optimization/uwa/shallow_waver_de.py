...
max_range_m = 5000
freq_hz = 500
wavelength = 1500 / freq_hz
max_range_wl = max_range_m / wavelength

xi_bound = fm.sin(35/180*fm.pi)**2
xi_bounds = (-xi_bound, 0)
bounds_ga = [(-15, 15)] * (pade_order[0] + pade_order[1]) * 2
...
errors_de = approx_error(num_coefs, den_coefs, xi_grid_2d.flatten(), dr_wl).reshape(shape) * (max_range_wl / dr_wl)

plt.imshow(
    np.log10(abs(errors_de)) < -2,
...
src = GaussSource(freq_hz=freq_hz, depth=30, beam_width=5, eval_angle=-30)
...
sspe_propagator = UnderwaterAcousticsSSPadePropagator(src=src, env=env, max_range_m=max_range_m, max_depth_m=200, comp_params=sspe_comp_params)
...
de_propagator = UnderwaterAcousticsSSPadePropagator(src=src, env=env, max_range_m=max_range_m, max_depth_m=200, comp_params=de_comp_params)
