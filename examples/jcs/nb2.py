...
def compute(order, dx_wl, dz_wl, method, max_propagation_angle, max_range, pyramid_angle):
    wl = 0.1
    ...
    f.colorbar(im, shrink=0.6, location='right')
    #f.tight_layout()
    plt.show()
    ...
    plt.plot(kz_arr, (np.imag(k_x_ratinterp)), label='imag ' + method)
    plt.xlabel('k_z')
    plt.ylabel('k_x abs. error')
    ...
    plt.plot(kz_arr, (np.real(k_x_ratinterp)), label='real ' + method)
    plt.xlabel('k_z')
    plt.ylabel('k_x abs. error')
    plt.xlim([kz_arr[0], kz_arr[-1]])
    # plt.ylim([1e-10, 1e-1])
