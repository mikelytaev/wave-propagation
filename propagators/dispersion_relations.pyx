...
def discrete_k_x_z_order2(double k, double dx, np.ndarray[complex, ndim=1] pade_coefs_num, np.ndarray[complex, ndim=1] pade_coefs_den, double dz, double kz):
    cdef double d_2 = sin(kz * dz / 2) ** 2
    cdef complex sum = 0
    cdef complex a_i = 0
    cdef Py_ssize_t i
    for i in range(0, len(pade_coefs_den)):
        if i < len(pade_coefs_num):
            ...
            a_i = 0
        sum += log((1 - 4 * a_i / (k * dz) ** 2 * d_2) / (1 - 4 * pade_coefs_den[i] / (k * dz) ** 2 * d_2))
